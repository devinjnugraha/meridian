...
export const config = {
    ...
    llm: {
        ...
        defaultModel: process.env.LLM_MODEL ?? "openrouter/healer-alpha",
...
/**
 * Compute bins_below based on pool volatility.
 * Linear scale from 35 (low vol) to config.strategy.binsBelow (high vol).
 *
 * Formula: round(35 + (volatility/5) * (binsBelow - 35)), clamped to [35, binsBelow]
 *
...
export function computeBinsBelow(volatility) {
    ...
    const range = maxBins - 35;
    return Math.max(35, Math.min(maxBins, Math.round(35 + (volatility / 5) * range)));
}
